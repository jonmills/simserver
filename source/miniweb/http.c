...
static int string_compare_no_case(const void* buf1, const void *buf2,unsigned int size);
...
int _mwParseHttpHeader(HttpSocket* phsSocket)
{
	...
	for(;;) {
		...
		switch (*(p++)) {
		case 'C':
			if (!string_compare_no_case(p,"onnection: ",11)) {
				p+=11;
				if (!string_compare_no_case(p,"close",5)) {
					SETFLAG(phsSocket,FLAG_CONN_CLOSE);
					p+=5;
				} else if (!string_compare_no_case(p,"Keep-Alive",11)) {
					CLRFLAG(phsSocket,FLAG_CONN_CLOSE);
					p+=11;
				}
			} else if ((!string_compare_no_case(p, "ontent-Length: ", 15)) || (!memcmp(p, "ontent-length: ", 15))){
				p+=15;
				p+=_mwGrabToken(p,'\r',buf,sizeof(buf));
				phsSocket->response.contentLength=atoi(buf);
			} else if (!string_compare_no_case(p, "ontent-Type: ", 13)) {
				p += 13;
				if (!string_compare_no_case(p, "multipart/form-data; boundary=", 30)) {
					...
					for (; *p != '\r'; p++) {
						if (!string_compare_no_case(p, "; filename=", 11)) {
							p += 11;
							p += _mwGrabToken(p ,'\r', buf, sizeof(buf));
							phsSocket->pxMP = calloc(1, sizeof(HttpMultipart));
			...
			break;
		case 'R':
			if (!string_compare_no_case(p,"eferer: ",8)) {
				p+=8;
				phsSocket->request.ofReferer=(int)(p - (char*)phsSocket->buffer);
			} else if (!string_compare_no_case(p,"ange: bytes=",12)) {
				int iEndByte;
				p+=12;
			...
			break;
		case 'H':
			if (!string_compare_no_case(p,"ost: ",5)) {
				p+=5;
...
static int string_compare_no_case(const void* buf1, const void *buf2,unsigned int size) {
        ...
        for(i=0;i<size;i++) {
                if (tolower(x[i])!=tolower(y[i])) break;
        }

        if (i<size) {
                if (tolower(x[i])>tolower(y[i])) return 1; else return -1;
        }
        else return 0;
}
